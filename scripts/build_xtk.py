...
not this project's ("Mockba DX7", see src/dx7_host.cpp's --client default).
`blank_mapping()`/`fix_midi_routes()` below scrub both; `audit()` refuses to
build if any leftover donor-addon string survives.

Usage:
    python3 scripts/build_xtk.py                            # -> addon/Force DX7 Control.xtk (+ .xtk.json)
    python3 scripts/build_xtk.py --track-name "MY TRACK"     # if you don't use "DX7 CTRL"
    python3 scripts/build_xtk.py --pack path/to/edited.json --out "addon/Force DX7 Control.xtk"
...
HOST_PARAMS_PATH = HERE.parent / "src" / "dx7_host.cpp"
HEADER = "ACVS\n3.3.0.0\nSerialisableTrackData\njson\nLinux\n"
...
def blank_mapping(doc):
    ...
    bank's 16 knobs the way customQLinks is -- the web GUI exposes every
    chain_param the host listens to, so this on-screen page should too. The
    first 16 slots get the Q-Link-covered CCs in KNOBS, reordered for
    correct top-to-bottom reading on page 1 (hardware-confirmed -- see
    ...
    for entry in mapping:
        ...
        entry["value"] = 0.0
        entry["name"] = ""
    for slot, (_key, label, cc) in zip(mapping, display_order(KNOBS)):
...
def fix_midi_routes(doc, control_channel):
    ...
    CORRECTED: the client name dx7_host.cpp passes to RtMidi ("Mockba DX7")
    is NOT what actually shows up in ALSA/mido's port list on real
    ...
    "DX7:In (Mockba)" / "DX7:Out (Mockba)" (an ephemeral numeric
    client:port suffix follows, e.g. "... 138:0", not included here since
    it's assigned fresh every boot). The previous version of this function
    used "Mockba DX7" verbatim, which was never actually correct on this
    device.
    ...
    behavior.
    """
    ch0 = control_channel - 1  # dx7_host.cpp/--control-channel are 1-based; the numeric
...
def main():
    ...
    ap.add_argument("--control-channel", type=int, default=1,
                     help="1-16, must match dx7_host's --control-channel (default 1) -- "
