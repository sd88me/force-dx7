#!/bin/sh
# ForceDX7 AddOn Manager (MockbaMod convention).
...
STOP() {
    for p in $(ps 2>/dev/null | grep "[d]x7_host" | awk '{print $1}'); do
        kill -9 $p 2>/dev/null
    done
}
...
if [ "$mode" = "DISABLE" ]; then
    STOP
    echo "$appTitle's dx7_host stopped (files kept - this addon has no boot-time footprint to remove)."
    exit 0
fi
...
if [ "$mode" = "ENABLE" ]; then
    ...
    echo
    echo "Runs fine with no setup (a quiet built-in 'Init' patch) - drop"
    echo "your own DX7-compatible .syx bank files (32 voices each) into"
    echo "  $installroot/banks/"
    echo "for real patches. Rescanned automatically, no restart needed to"
    echo "pick up a new bank dropped in later."
    exit 0
fi
