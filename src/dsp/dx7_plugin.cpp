...
static int v2_get_param(void *instance, const char *key, char *buf, int buf_len) {
    ...
    if (strcmp(key, "patch_list") == 0) {
        /* MockbaMod/Force port addition, not upstream (same one added to
         * force-jv880's jv880_plugin.cpp for the same reason): get_param
         * only ever exposed the CURRENTLY loaded patch's name via
         * preset_name/patch_name below. Every patch's real name in the
         * active bank is already sitting in inst->patch_names[], populated
         * once when the bank is scanned, just never serialized out. Same
         * JSON shape/style as the existing syx_bank_list key just below. */
        int written = snprintf(buf, buf_len, "[");
        for (int i = 0; i < inst->preset_count && written < buf_len - 50; i++) {
            if (i > 0) written += snprintf(buf + written, buf_len - written, ",");
            written += snprintf(buf + written, buf_len - written,
                "{\"index\":%d,\"name\":\"%s\"}", i, inst->patch_names[i]);
        }
        written += snprintf(buf + written, buf_len - written, "]");
        return written;
    }
